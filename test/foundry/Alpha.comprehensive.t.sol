...
contract AlphaComprehensiveTest is Test {
    ...
    function test_Create_IncreasesCount() public {
        ...
        alpha.create(address(targetToken), 1 ether, block.timestamp + ONE_HOUR, block.timestamp + ONE_DAY, block.timestamp + ONE_HOUR - 60, 10000);
        assertEq(alpha.count(), 1);

        alpha.create(address(targetToken), 2 ether, block.timestamp + ONE_HOUR, block.timestamp + ONE_DAY, block.timestamp + ONE_HOUR - 60, 10000);
    ...
    function test_Join_EmitsEvent() public {
        uint256 tradeId = alpha.create(address(targetToken), 5 ether, block.timestamp + ONE_HOUR, block.timestamp + ONE_DAY, block.timestamp + ONE_HOUR - 60, 10000);
    ...
    function test_Join_AtExactDeadline() public {
        ...
        uint256 tradeId = alpha.create(address(targetToken), 5 ether, deadline + 60, block.timestamp + ONE_DAY, deadline, 10000);
    ...
    function test_Join_SameUserMultipleTimes() public {
        uint256 tradeId = alpha.create(address(targetToken), 10 ether, block.timestamp + ONE_HOUR, block.timestamp + ONE_DAY, block.timestamp + ONE_HOUR - 60, 10000);
    ...
    function test_Join_AfterBought() public {
        uint256 buyTime = block.timestamp + ONE_HOUR;
        uint256 tradeId = alpha.create(address(targetToken), 1 ether, buyTime, block.timestamp + ONE_DAY, buyTime - 60, 10000);
    ...
    function test_ExecuteBuy_EmitsEvent() public {
        uint256 buyTime = block.timestamp + ONE_HOUR;
        uint256 tradeId = alpha.create(address(targetToken), 5 ether, buyTime, block.timestamp + ONE_DAY, buyTime - 60, 10000);
    ...
    function test_ExecuteSell_EmitsEvent() public {
        ...
        uint256 tradeId = alpha.create(address(targetToken), 5 ether, buyTime, sellTime, buyTime - 60, 10000);
    ...
    function test_Withdraw_SetsStatusExpired() public {
        uint256 deadline = block.timestamp + ONE_HOUR;
        uint256 tradeId = alpha.create(address(targetToken), 100 ether, deadline + 60, block.timestamp + ONE_DAY, deadline, 10000);
    ...
    function test_GetTradeInfo_AllFields() public {
        ...
        uint256 tradeId = alpha.create(address(targetToken), 5 ether, buyTime, sellTime, deadline, 10000);
    ...
    function test_TradesArray_DirectAccess() public {
        alpha.create(address(targetToken), 5 ether, block.timestamp + ONE_HOUR, block.timestamp + ONE_DAY, block.timestamp + ONE_HOUR - 60, 10000);
