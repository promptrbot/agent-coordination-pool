...
- Swaps ETH → target token via Uniswap V4
...
- Swaps target token → ETH via Uniswap V4
...
Alpha uses **Uniswap V4 Universal Router** for swaps. Works with any V4 pool — Clanker tokens, standard pairs, custom hooks.

**Pool identification:**
- Uses PoolKey struct (currency0, currency1, fee, tickSpacing, hooks)
- Swaps via Permit2 + Universal Router encoded commands
- Compatible with all tokens deployed through standard tooling (Clanker, etc.)
